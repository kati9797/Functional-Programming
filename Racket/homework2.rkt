...
(define (curr-res) (λ(fs xs y) ((first fs) (first xs) ((first (rest fs)) (first (rest xs)) y))))
(define (last-res) (λ(fs xs y) ((first fs) (first xs) (id y))))
(define (id n) n)

(define (pair-compose fs xs)
  (define (helper sum fs xs y)
    (cond [(= 1 (length xs)) (+ sum ((last-res) fs xs y))]
          [(empty? xs) sum]
          [else (helper (+ sum ((curr-res) fs xs y)) (drop fs 2) (drop xs 2) y)]))
  (λ(y) (helper 0 fs xs y)))
